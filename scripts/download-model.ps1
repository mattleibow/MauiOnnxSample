...
$ModelDir = Join-Path $RepoRoot "MauiOnnxSample\Resources\Raw\Models\phi-3.5-mini"

Write-Host "Downloading Phi-3.5-mini-instruct ONNX model..."
...
if (Get-Command "huggingface-cli" -ErrorAction SilentlyContinue) {
    ...
    huggingface-cli download microsoft/Phi-3.5-mini-instruct-onnx `
        ...
        --local-dir "$env:TEMP\phi35-onnx"
    
    $src = Join-Path "$env:TEMP\phi35-onnx" "cpu-int4-rtn-block-32-acc-level-4"
...
src = os.path.join(tmp, 'cpu-int4-rtn-block-32-acc-level-4')
dst = r'$ModelDir'
...
"@
    ...
    Write-Host "  https://huggingface.co/microsoft/Phi-3.5-mini-instruct-onnx/tree/main/cpu-int4-rtn-block-32-acc-level-4"
